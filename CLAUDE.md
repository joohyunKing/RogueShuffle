...
src/
  ...
  assets/
    ...
    images/
      ...
      relic/            # 유물 이미지
      debuff/           # 디버프 이미지
      gimmick/          # 기믹 아이콘 이미지 (aoe.png, suit.png, under4.png, def.png)
      bg/               # 라운드별 배경 이미지
...
| `bgm_market` | Cobblestone_Waltz.mp3 |
...
| `ui_battle_popup_v` | 배틀 클리어 팝업 배경 (세로형) |
...
- 기믹: `gimmick_${gimmick.id}` — gimmicks.json의 img 필드
...
context = {
  ...
  attrs: {},            // 슈트별 레벨 { S, H, C, D } — 슈트 적응 보너스 계산용
  adaptability: {},     // 슈트별 적응도 { S, H, C, D }
}
```
...
> **슈트 적응 점수 보너스**: 족보에 포함된 각 카드의 baseScore에 `floor((attrs[suit]-1) × adaptability[suit])` 추가.
> scoreService.js 내부에서 red_seal 적용과 같은 시점에 계산 → 배수 적용 전에 합산됨.
...
// 반환: 몬스터 객체 배열 { id, name, race, job, hp, maxHp, atk, def, skill, sprite, xp, gold, gimmick? }
```

- **일반 몬스터**: `roundData.races`에 해당하는 종족만 풀링, `totalCost` 이내로 조합
  - 같은 몬스터 최대 2마리 제한
  - elite 배틀: cost 2 몬스터 가중치 증가
- **elite 기믹**: 엘리트 배틀 몬스터에 `_assignGimmick()` 호출 → `monster.gimmick` 객체 설정
  - `suit_resist` 타입은 랜덤 슈트 `gimmick.suit` 지정 + description의 `{suit}` 치환
  - `first_turn_def` 타입은 `gimmick.firstTurnActive = true` 설정 (TURN END 시 BattleScene에서 false로)
...
- effects type 주요 목록:
  - `add` / `addPerHandUsage` / `addPerTotalHandUsage` — 카드/족보 기반 점수 가산
  - `addPerExcessDeck` — 덱이 threshold 초과 시 초과 카드 1장당 `value`점 가산 (avarice 유물)
  - `sideAmplify` — 좌우 이웃 유물 효과를 `value`배 증폭 (side_mirror)
  - `verticalAmplify` — 위아래 이웃 유물 효과를 `value`배 증폭 (vertical_mirror, 9슬롯 기준 idx±3)
```js
...
### scoreService 점수 파이프라인
1. **카드별 baseScore 계산** (족보 구성 카드만):
   - 카드 baseScore
   - + red_seal 보너스 (+20)
   - + 슈트 적응 보너스 `floor((attrs[suit]-1) × adaptability[suit])` (ctx.attrs가 있을 때)
2. **유물 amplifier map** (`buildAmplifierMap`):
   - `sideAmplify`: 좌우(col±1) 이웃 유물 효과를 `value`배 (side_mirror)
   - `verticalAmplify`: 위아래(idx±3) 이웃 유물 효과를 `value`배 (vertical_mirror)
3. **ADD_TYPES** (`add`, `addPerHandUsage`, `addPerTotalHandUsage`, `addPerExcessDeck`): amplifier 적용 후 baseScore에 합산
4. **족보 배수(multi)** 적용
5. **final scope** 유물 적용
...
- 몬스터 간격: `MonsterManager.calcMonsterPositions()` — **일반** 최대 130px / **엘리트** 최대 210px, margin 100px/80px
- 족보 있을 때 몬스터에 "ATTACK!" 힌트 + 히트 영역 표시
- 몬스터 스프라이트: `MonsterView` (tween 기반 idle 부유 애니메이션)
- **엘리트 기믹 아이콘**: HP바 왼쪽에 `gimmick_${id}` 이미지 표시, hover/click → TooltipUI 툴팁
...
> `onTurnEnd()`에서 `first_turn_def` 기믹 비활성화:
> 살아있는 몬스터 중 `gimmick.type === 'first_turn_def'`이면 `gimmick.firstTurnActive = false` 설정.
> MonsterView는 이 값에 따라 기믹 아이콘 alpha(1.0/0.35)를 조정.
...
### 엘리트 기믹 처리
- `_getEffectiveDef(mon)`: `first_turn_def` 기믹이 active면 `def * defMultiplier` 반환, 아니면 `def`
- `_applyGimmickResist(mon, damage, attackCtx)`: 기믹 타입별 50% 데미지 감소 판정
  - `aoe_resist`: `attackCtx.isAoe === true`일 때 감소
  - `suit_resist`: `attackCtx.suitCounts[gimmick.suit] > 0`일 때 감소
  - `small_hand_resist`: `attackCtx.cardCount < gimmick.threshold`일 때 감소
  - `first_turn_def`: 데미지 감소 없음 (DEF 배율만 적용)
- `attackCtx = { isAoe, suitCounts, cardCount }` — MonsterManager 내부에서 생성해 전달

### 씰 효과 적용 범위 주의
- `scene._applySealEffects?.(details.cards)` — **족보 구성 카드만** (details.cards) 씰 효과 적용
- 선택된 전체 카드(selectedCards) 기준으로 하면 족보 외 카드에도 씰 효과가 적용되는 버그 발생
...
view.update(mon, x, canBeTarget)  // 위치·HP·스탯 갱신 + ATTACK! 표시 + 기믹 아이콘 갱신
...
view.destroy()     // 모든 오브젝트 제거 (기믹 툴팁 포함)
```

### 기믹 아이콘
- HP 바 프레임 왼쪽에 배치 (`_gimmickIconOffsetX = -(HP_FRAME_HALF + 12)`)
- 기믹 있을 때만 `setVisible(true)`, `first_turn_def`가 비활성이면 alpha 0.35
- `pointerover` / `pointerdown` → `_showGimmickTip()` (TooltipUI, 제목 색 `#ffcc44`)
- `pointerout` → `_hideGimmickTip()`

---

---

## 엘리트 기믹 시스템 (data/gimmicks.json)
엘리트 배틀 전용. 스폰 시 각 몬스터에 랜덤 기믹 1개 배정.

| id | 이름 | 타입 | 효과 |
|----|------|------|------|
| `aoe_resist` | 광역 저항 | `aoe_resist` | 광역 공격 데미지 50% 감소 |
| `suit_resist` | 슈트 저항 | `suit_resist` | 특정 슈트 공격 데미지 50% 감소 (랜덤 슈트) |
| `small_hand_resist` | 소수 저항 | `small_hand_resist` | 4장 미만 공격 시 데미지 50% 감소 |
| `first_turn_def` | 초반 철벽 | `first_turn_def` | 첫 턴에만 DEF 3배 적용 |

- 기믹 이미지: `gimmick_${id}` (PreloadScene에서 gimmicks.json 기반 동적 로드)
- `suit_resist`: 스폰 시 `gimmick.suit`에 랜덤 슈트 배정, description의 `{suit}` 심볼로 치환
- `first_turn_def`: `gimmick.firstTurnActive = true` → TURN END 시 `false`로 비활성화
